...
\begin{itemize}
\item \textbf{Trigger}: \sidenotehighlight{update}{\draw node[anchor=north west] (target) at (.5,-.3) {from $G_{r-\epsilon}$ to $G_{r+\epsilon}$} (mymark) to[out=-90,in=170] ([yshift=-7pt]target.north west);} when $r=f(v)$ for some vertex $v$.\\
