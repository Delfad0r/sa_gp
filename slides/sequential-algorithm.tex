...
\begin{itemize}
\item<5-> \textbf{Trigger}: \sidenotehighlight<5>{update}{\draw node[anchor=north west] (target) at (.5,-.3) {from $r=f(v)-\epsilon$ to $r=f(v)+\epsilon$} (mymark) to[out=-90,in=170] ([yshift=-7pt]target.north west);} when processing a vertex $v$\\
...
\end{itemize}
\onslide<14->{
$\leadsto$ \sidenotehighlight<14->{offline}{} dynamic connectivity problem \onslide<15->{$\leadsto$ \sidenotehighlight<15>{\textbf{ST-trees}}{\draw node[anchor=north east] (target) at (-.5,-.3) {support all the operations in $O(\log m)$} (mymark) to[out=-90,in=10] ([yshift=-7pt]target.north east);}}}
\end{itemize}
\end{block}
\end{frame*}
\end{document}
