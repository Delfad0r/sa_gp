...
\begin{tikzpicture}
\matrix[cells={scale=.5},row sep=7]{
\draw[fill=gray!40,line cap=round] (-1,1) arc(-180:0:1) -- cycle;
\draw[fill=gray] (0,1) circle(1 and .25);
\draw[fill=white] circle(3pt);\\
\draw[fill=gray!40,line cap=round] (-1,-1) arc(180:0:1) arc(0:-180:1 and .25);
\draw[dashed,opacity=.5] (-1,-1) arc(180:0:1 and .25);
\draw[fill=white] circle(3pt);\\
...
\draw[name path=second] (-1,-.2) arc(-90:90:.6 and .3) (1,-.2) arc(270:90:.6 and .3);
\draw[dashed,opacity=.5,intersection segments={of={first and second}, sequence={L2}}];
\draw[intersection segments={of={first and second}, sequence={L1 L3}}];
